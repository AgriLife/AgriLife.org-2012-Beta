			...
			<?php if ( have_posts() ) : ?>
				...
				<header class="page-header">
					<h1 class="page-title section-title">
						<?php if ( is_day() ) : ?>
							<?php printf( __( 'Daily Archives: %s', 'flexopotamus' ), '<span>' . get_the_date() . '</span>' ); ?>
						<?php elseif ( is_month() ) : ?>
							<?php printf( __( 'Monthly Archives: %s', 'flexopotamus' ), '<span>' . get_the_date( 'F Y' ) . '</span>' ); ?>
						<?php elseif ( is_year() ) : ?>
							<?php printf( __( 'Yearly Archives: %s', 'flexopotamus' ), '<span>' . get_the_date( 'Y' ) . '</span>' ); ?>
						<?php else : ?>
